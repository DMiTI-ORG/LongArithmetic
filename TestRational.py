...
class TestRational(TestCase):
    ...
    def test_reduce(self, quotient, gcd, abs):
        ...
        quotient.side_effect = [IntegerNumber(0, 1, [2]), IntegerNumber(0, 1, [1])]
        self.assertEqual(RationalNumber((0, 1, [2]), (1, [1])), number_1.reduce())

    def test_is_integer(self):
        number_one = RationalNumber([1, 4, [1, 2, 3, 9]], [3, [4, 1, 3]])  # -1239 413
        self.assertEqual(str(number_one.is_integer()), 'True')

        number_two = RationalNumber([1, 3, [1, 2, 3]], [3, [1, 2, 3]])  # -123 123
        self.assertEqual(str(number_two.is_integer()), 'True')

        number_free = RationalNumber([0, 4, [1, 2, 3, 9]], [5, [1, 5, 4, 1, 3]])  # 1239 15413
        self.assertEqual(str(number_free.is_integer()), 'False')

        number_four = RationalNumber([0, 1, [0]], [5, [1, 5, 4, 1, 3]])  # 0 15413
        self.assertEqual(str(number_four.is_integer()), 'True')

        number_five = RationalNumber([1, 3, [1, 0, 0]], [1, [2]])
        self.assertEqual(str(number_five.is_integer()), 'True')  # -100 2

        number_six = RationalNumber([0, 6, [1, 2, 3, 2, 4, 6]], [3, [1, 2, 3]])  # 123246 123
        self.assertEqual(str(number_six.is_integer()), 'True')

    def test_integer_to_rational(self):
        ...
        testnum1 = IntegerNumber(1, 1, [0])
        testnum2 = IntegerNumber(0, 2, [3, 4])
        testnum3 = IntegerNumber(1, 2, [3, 4])
        testnum4 = IntegerNumber(0, 1, [0])
        self.assertEqual(RationalNumber.integer_to_rational(testnum1), number1)
        self.assertEqual(RationalNumber.integer_to_rational(testnum2), number2)
        self.assertEqual(RationalNumber.integer_to_rational(testnum3), number3)
        self.assertEqual(RationalNumber.integer_to_rational(testnum4), number4)

    def test_to_integer(self):
        number_1 = RationalNumber((1, 4, [2, 3, 4, 5]),(1,[1]))  # -2345/1
        number_2 = IntegerNumber(1, 4, [2, 3, 4, 5])  # -2345
        self.assertEqual(str(number_2), str(number_1.to_integer()))

        number_1 = RationalNumber((1, 6, [2, 6, 4, 8, 3, 9]), (1, [1]))  # -264839/1
        number_2 = IntegerNumber(1, 6, [2, 6, 4, 8, 3, 9])  # -264839
        self.assertEqual(str(number_2), str(number_1.to_integer()))

        number_1 = RationalNumber((0, 8, [2, 4, 8, 1, 2, 9, 3, 6]), (1, [1]))  # 24812936/1
        number_2 = IntegerNumber(0, 8, [2, 4, 8, 1, 2, 9, 3, 6])  # 24812936
        self.assertEqual(str(number_2), str(number_1.to_integer()))

        number_1 = RationalNumber((1, 6, [2, 6, 4, 8, 3, 9]), (1, [7]))  # -264839/7
        self.assertEqual('ERROR', str(number_1.to_integer()))

        number_1 = RationalNumber((0, 3, [4, 2, 9]), (1, [0]))  # 429/0
        self.assertEqual('ERROR', str(number_1.to_integer()))

    @patch.object(IntegerNumber, 'add')
    ...
    def test_add(self, lcm, quotient, multiply, add):
        ...
        multiply.side_effect = [IntegerNumber(0, 2, [4, 5])]
        multiply.side_effect = [IntegerNumber(0, 2, [6, 0])]
        add.side_effect = [IntegerNumber(0, 3, [1, 0, 5])]
    ...
    def test_subtract(self, quotient, lcm, subtract, multiply):
        # 1/2 - 1/3 = 1/6
        multiply.side_effect = [IntegerNumber(0, 1, [3]), IntegerNumber(0, 1, [2])]
        subtract.return_value = IntegerNumber(0, 1, [1])
        ...
        # 5/6 - 1/1 = -1/6
        multiply.side_effect = [IntegerNumber(0, 1, [5]), IntegerNumber(0, 1, [6])]
        subtract.return_value = IntegerNumber(1, 1, [1])
        lcm.return_value = NaturalNumber(1, [6])
        ...
        multiply.side_effect = [IntegerNumber(0, 2, [3, 3]), IntegerNumber(0, 2, [1, 4])]
        subtract.return_value = IntegerNumber(0, 2, [1, 9])
    ...
    def test_multiply(self, multiply2, multiply1):
        ...
        multiply1.return_value = IntegerNumber(0, 3, [3, 2, 2])
        ...
        multiply1.return_value = IntegerNumber(0, 2, [7, 0])
    ...
    def test_divide(self, multiply):
        ...
        multiply.side_effect = [IntegerNumber(1, 3, [5, 5, 1]), IntegerNumber(0, 4, [2, 2, 3, 6])]
        ...
        multiply.side_effect = [IntegerNumber(1, 4, [4, 4, 5, 5]), IntegerNumber(1, 4, [1, 1, 0, 0])]
